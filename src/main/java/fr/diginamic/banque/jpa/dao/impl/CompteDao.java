...
public class CompteDao extends Dao implements Idao<Compte> {
	...
	public boolean update(Compte e) throws Exception {
		...
		try {
			em = fd.getEm();
			em.getTransaction().begin();
			Compte ecli = em.find(Compte.class, e.getId());
			if (ecli != null) {
				ecli.setNumero(e.getNumero());
				ecli.setSold(e.getSold());
				ecli.setCompteLivretA(e.getCompteLivretA());
				ecli.setCompteAssuranceVie(e.getCompteAssuranceVie());
				ecli.setCompteOperation(e.getCompteOperation());
				em.merge(ecli);
				em.getTransaction().commit();
				return true;
			}
