...
public class OperationDao extends Dao implements Idao<Operation> {
	...
	public boolean update(Operation e) throws Exception {
		...
		try {
			em = fd.getEm();
			em.getTransaction().begin();
			Operation ecli = em.find(Operation.class, e.getId());
			if (ecli != null) {
				ecli.setDate(e.getDate());
				ecli.setMontant(e.getMontant());
				ecli.setMotif(e.getMotif());
				ecli.setOperationVirement(e.getOperationVirement());
				em.merge(ecli);
				em.getTransaction().commit();
				return true;
			}
